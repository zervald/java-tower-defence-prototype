...
public class DescripteurTour {
  ...
  public DescripteurTour(
      String nom, int prix, Function<PositionTuile, Tour> fonctionConstructionTour, int posY) {
    this.nom = nom;
    this.prix = prix;
    ...
    this.positionBouton_tuile =
        new PositionTuile(Constantes.POSITION_CARACTERISTIQUE_X_BOUTON, posY);
    this.positionBouton_pixel = this.positionBouton_tuile.positionPixel();
    this.positionCout_tuile =
        new PositionTuile(Constantes.POSITION_CARACTERISTIQUE_X_NOMBRE, posY + 1);
  }

  public BiConsumer<Jeu, PositionTuile> acheterTour =
      (jeu, position) -> {
        if (prix <= jeu.argent) {
          jeu.argent -= prix;
